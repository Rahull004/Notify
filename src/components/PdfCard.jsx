  ...
  return (
    <div className="flex">
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 h-screen">
        <div className="bg-white py-4 rounded-xl shadow-xl w-3/4 md:w-3/4 h-5/6 overflow-hidden px-16 pt-10">
          <div className="mb-3 mt-2 pb-6 text-xl font-semibold">
            <label>
              Upload Files
              <input
                id="pdf-upload"
          ...
          <div className="h-[87%] w-full flex gap-3">
            <div
              className="text-center w-[60%] border-dashed border-2 border-orange-300	0 p-10 bg-amber-100	rounded-xl h-5/6 flex flex-col items-center justify-center"
            ...
            <div className=" overflow-auto flex justify-center items-center h-12">
              {pdfs.map((pdf, index) => (
                <Link
                  to={`/pdfviewer/${pdf.$id}`}
                  key={pdf.$id}
                  onClick={() => handlePdfClick(pdf.$id)}
                  className="cursor-pointer max-w-20 mx-2 rounded-lg p-2 bg-red-600"
                >
          ...
          <div className="relative">
            <button
              className="absolute bottom-0 right-0 bg-red-500 hover:bg-red-700 text-white font-bold py-3 px-5 rounded"
